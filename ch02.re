 ...
 * 画像: @<img>{img01}
 * テーブル: @<table>{tbl01}
 * 見出し: @<hd>{装飾用インライン} または @<hd>{inline_ref_id}
 * コラム見出し: @<column>{コラムのサンプル}

見出しについては、「@<tt>|@<hd>{装飾用インライン}|」といったようにタイトルを記述するのが基本ですが、「@<tt>{=={some_id\} 節見出し}」のようにIDを振ることもできます。その場合は「@<tt>|@<hd>{some_id}|」というように参照します。
...
==={inline_ref_id} インライン参照用のサンプルその2

参照系のインラインでIDを使って参照するのに使っているものです。
